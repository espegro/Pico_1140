...
[[noreturn]] static void disk_io_failure(const char *operation) {
    status_rgb_error();
    printf("RK05 internal flash %s failed\r\n", operation);
    while (true) {}
}
...
void RK11::readwrite() {
    ...
    for (i = 0; i < 256 && rkwc != 0; i++) {
        ...
        if (w) {
            ...
            if (!rk05.write(buf, 2, &bcnt))
                disk_io_failure("write");
        }
	    else {
		    uint8_t buf[2];
            if (!rk05.read(buf, 2, &bcnt))
                disk_io_failure("read");
    ...
    if (w && i<256) {                           // Fill remainder of sector with zero
        for (i=i;i<256;i++)
            if (!rk05.write(&zero, 2, &bcnt))
                disk_io_failure("write");
    }

    if (w && !rk05.sync())
        disk_io_failure("sync");
    sector++;
    if (sector > 013) {
        sector = 0;
        surface++;
